...
import lab8.customs.CustomLatch;
import lab8.customs.CustomSemaphore;
...
public class MultiThreadingPiCalculating {
    ...
    public double calculatePI() throws ExecutionException, InterruptedException {
        ...
        Semaphore semaphore = new CustomSemaphore(threadCount / 2);
        CountDownLatch latch = new CustomLatch(threadCount);
            ...
            Future<Double> future =
                    pool.submit(
                            () -> {
                                ...
                                try {
                                    semaphore.acquire();
                                    result = PiCalculating.calc(from.getValue(), to.getValue(), printer);
